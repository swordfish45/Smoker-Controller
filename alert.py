...
def sendmail(
    fromaddr, toaddr, username, password, email_body, email_subject, smtpsrv, smtpport
):
    # Build the email
...
def get_temperature(sensnum):
    ...
    cur = con.cursor()
    cur.execute(
        "SELECT (MAX(timestamp)*1000) as timestamp, sensnum, temp from temps where sensnum = ?",
        (sensnum,),
    )
...
for sensor in xrange(0, 3):
    ...
    for alert in alerts:
        ...
        if ConfigSectionMap("Config")[alert] == "0":
            ...
            continue
            # check high alerts
        if re.search("high", alert) and int(SensTemp) > int(
            ConfigSectionMap("Config")[alert]
        ):
            ...
            if int(holddown[count]) == int(ConfigSectionMap("Config")["alerthold"]):
                msg += (
                    ...
                    + str(Sensnum)
                    + " has exceeded "
                    + str(ConfigSectionMap("Config")[alert])
                    + " degrees F\n"
                )
                ...
                count += 1
                # check low alerts
        elif re.search("low", alert) and int(SensTemp) < int(
            ConfigSectionMap("Config")[alert]
        ):
...
if msg != "":
    ...
    sendmail(
        ConfigSectionMap("Config")["smtpemail"],
        Email,
